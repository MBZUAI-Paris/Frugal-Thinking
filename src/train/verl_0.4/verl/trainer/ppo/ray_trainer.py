...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        # to randomly sample k test cases on the fly for fast training on code data
        self.sample_k_code_test_case = sample_k_code_test_case
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # do the sampling of the test cases here on batch_dict where ability=code
                # Convert back to numpy object array so DataProto.from_single_dict receives the expected type.
                batch_dict["reward_model"] = np.array(
                    [
                        sample_test_cases(data_item=data_item, k=self.sample_k_code_test_case)
                        if batch_dict["ability"][i] == "code" else data_item
                        for i, data_item in enumerate(batch_dict["reward_model"])
                    ],
                    dtype=object,
                )
...
def sample_test_cases(
    ...
    """
    Sub‑sample up to *k* test cases in the data_item["ground_truth"] field.
