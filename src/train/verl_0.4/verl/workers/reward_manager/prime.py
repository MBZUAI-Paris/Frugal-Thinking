...
async def single_compute_score(evaluation_func, completion, reference, task, task_extra_info, executor, ability, timeout=300.0):
    ...
    try:
        ...
        future = loop.run_in_executor(
            ...
            partial(
                ...
                ability=ability,    # ensure correct positional mapping
        ...
        print(f"[Error] Task failed: {e}, completion: {completion}")
        return None  # Default value for failed rows


async def parallel_compute_score_async(
    evaluation_func, completions, references, tasks, extra_info=None, ability=None, num_processes=96
):
    ...
    with ProcessPoolExecutor(max_workers=num_processes) as executor:
        ...
        try:
            ...
            tasks_async = [
                single_compute_score(evaluation_func, c, r, t, ei, executor, a, timeout=300.0)
                for c, r, t, ei, a in zip(completions, references, tasks, extra_info, ability)
            ]
...
def run_reward_scoring(evaluation_func, completions, references, tasks, extra_info=None, num_processes=96, ability=None):
    loop = asyncio.new_event_loop()
    num_processes = min(num_processes, os.cpu_count() // 2)
    asyncio.set_event_loop(loop)
    try:
        return loop.run_until_complete(
            parallel_compute_score_async(evaluation_func, completions, references, tasks, extra_info, ability, num_processes)
        )
    finally:
        loop.close()
...
class PrimeRewardManager(AbstractRewardManager):
    ...
    def __init__(
        ...
        print('[INFO] Using PrimeRewardManager...')
    ...
    def verify(self, data):
        ...
        # added ability to the compute_score function to handle the code-math case with ease
        ability = data.non_tensor_batch.get("ability", None)
        # print(f'[INFO-reward_manager.prime.py] ability: {ability}')
        # print(f'[INFO-reward_manager.prime.py] type(ability): {type(ability)}')
