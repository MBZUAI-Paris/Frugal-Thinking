...
def _normalize_ground_truth_values(ground_truth):
    """Return a list of candidate answers extracted from ``ground_truth``."""
    ...
    if isinstance(ground_truth, str):
        ...
        if stripped.startswith("[") and stripped.endswith("]"):
            ...
            if isinstance(parsed, (list, tuple, set)):
                return [str(item).strip() if item is not None else None for item in parsed]
        return [stripped]

    if isinstance(ground_truth, (list, tuple, set)):
        return [str(item).strip() if item is not None else None for item in ground_truth]

    return [str(ground_truth).strip()]


def _matches_ground_truth(answer, ground_truth):
    answer = answer.strip()
    for candidate in _normalize_ground_truth_values(ground_truth):
...
def compute_score(
    ...
    # maximum possible reward from tags
    R_ANSWER = 1 - R_TAG * len(tags)
    ...
    if only_if_correct_format:
        ...
        if think_present and answer_tag_present:
            ...
            answer_reward = R_ANSWER if answer is not None and _matches_ground_truth(answer, ground_truth) else 0.0
...
if __name__ == "__main__":
    ...
    # Multiple ground truths provided as a list
    solution = "<think>Equivalent forms</think><answer>\\boxed{3/4}</answer>"
    gt = ["0.75", "3/4"]
    reward = compute_score(solution, gt)
    print(f"Test reward with list GT: {reward}")  # Expected: 1.0

    # Multiple ground truths provided as a stringified list
    solution = "<think>Equivalent forms</think><answer>\\boxed{0.75}</answer>"
    gt = "['0.75', '3/4']"
    reward = compute_score(solution, gt)
    print(f"Test reward with stringified list GT: {reward}")  # Expected: 1.0
