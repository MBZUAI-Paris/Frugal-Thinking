...
def compute_score(solution_str, ground_truth, method="strict", only_if_correct_format=False):
    ...
    Args:
        ...
        only_if_correct_format: if True, the answer reward is given only when both
                                <think> and <answer> tags are present.
    """
    # --- tag‑based rewards ---------------------------------------------------
    tags = [
        "think",
    ...
    if only_if_correct_format:
        ...
        if think_present and answer_tag_present:
            ...
            answer_reward = 0.8 if answer is not None and _matches_ground_truth(answer, ground_truth) else 0.0
